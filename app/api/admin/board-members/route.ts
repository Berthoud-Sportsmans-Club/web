...
export async function POST(request: Request) {
  ...
  if (!name || !role || sortOrder == null) {
    return NextResponse.json({ error: 'Missing required fields' }, { status: 400 })
  }

  const [row] = await db.insert(boardMembers).values({ name, role, department: department || null, phone: phone || null, sortOrder: Number(sortOrder) }).returning()
